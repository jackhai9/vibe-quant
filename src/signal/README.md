...
`[SIGNAL]` INFO 日志按“语义变化 + 5 秒心跳”去重：`orderbook_price` 观察 `reason + roi_mult + accel_mult`，`orderbook_pressure` 观察 `reason + execution_preference + price_override`；盘口微抖保留在 DEBUG/原始市场数据里，不再直接刷满 console。<br>
输出 ExitSignal 给执行引擎。
