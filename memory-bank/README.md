...
<!-- Output: 设计/架构/进度索引与维护提示（含执行反馈、规则提取策略、自动发现持仓与发布准备记录） -->
...
设计/架构/进度的单一事实来源。<br>
记录关键决策与实施进度（含撤单接口分层、improve 信号触发路径、成交率反馈/TTL 反馈、自动发现持仓与运行时 symbols 管理、minNotional 放行与规则提取策略、日志规范、成交字段与发布准备记录）。<br>
重要变更需同步更新本目录。
