...
- 应用启动后会在后台扫描最近 `24h` 的 `PRESSURE_STATS` 日志，并额外带上一小段 pre-lookback warmup stats，再按当前 side-adjusted same-window 口径重放最近一段 regime
...
│   │   └── pressure_stats.py # orderbook_pressure 旁路统计（窗口聚合 + rolling regime 状态）
│   └── utils/
