...
| **SignalEngine** | 按 symbol 在 `orderbook_price` / `orderbook_pressure` 两条互斥路径间评估平仓条件；维护 prev/last trade price、盘口量 dwell 与来源 freshness；`orderbook_price` 计算 accel/ROI 倍数，`orderbook_pressure` 在 `lot_mult` 基础上按 `qty_jitter_pct` 随机抖动下单量 | MarketEvent, Position | ExitSignal |
