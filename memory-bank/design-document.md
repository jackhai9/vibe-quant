...
### 6.1 基准片大小
- `orderbook_price`：`base_qty = base_mult * minQty`（默认 `execution.base_mult=1`）
- `orderbook_pressure`：`base_qty = base_mult * minQty`（与 `orderbook_price` 共用 `execution.base_mult`）
...
- `orderbook_pressure` 与 `orderbook_price` 共用 `execution.base_mult` 作为基准片大小
- `roi_mult / accel_mult` 是公共 sizing modifiers；`orderbook_pressure` 默认继承 `execution.use_*`，也可通过 `pressure_exit.use_roi_mult` / `pressure_exit.use_accel_mult` 显式覆盖
