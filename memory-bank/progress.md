...
## Milestone/附加改进：operator safety and dry-run boundary
...
**动机**：为部分 symbol 引入与 `orderbook_price` 互斥的 `orderbook_pressure` 平仓路径，并补齐 `bookTicker/depth10` 双源 freshness、主动条件 dwell reset 与正式文档收口，避免用过期顶档量继续触发主动单。<br>
**产出**：
...
- `tests/test_post_only_retry.py`：覆盖 `orderbook_price` 的 post-only reject aggressive retry，以及 `orderbook_pressure` 被动单不降级为 taker 的约束
...
- `config/config.example.yaml` / `docs/configuration.md`：补充配置说明与阈值行为、日志多窗口输出（分钟单位，日志间隔取最小窗口一半；成交率以提交时间归因）
