...
> binance-exit-executor 详细配置参数说明与调优建议

---

## 概述

binance-exit-executor 使用 YAML 配置文件（`config/config.yaml`）管理所有运行参数。配置分为两个层级：

- **global**：全局默认配置，适用于所有交易对
- **symbols**：按交易对覆盖配置，优先级高于 global（运行时 symbols 自动发现，非白名单）
...
- **获取方式**:
  ...
  3. 找到 `"chat":{"id":"YOUR_CHAT_ID"}` 字段

#### VQ_LOG_DIR
- **类型**: `string`
- **默认值**: `logs/`（相对于工作目录）
- **说明**: 日志文件存储目录
- **systemd 部署**: 通常设置为 `/var/log/binance-exit-executor`
- **日志文件**: `binance-exit-executor_YYYY-MM-DD.log` 与 `error_YYYY-MM-DD.log`（旧日志压缩为 `.gz`）
...
- **用途**: 全局调整动态数量策略每次平仓的基础仓位大小
- **示例**: 设为 `2` 表示每次至少平 `2 × minQty`

##### use_roi_mult
- **类型**: `bool`
- **默认值**: `true`
- **说明**: 动态数量策略是否启用公共 `roi_mult`
- **用途**: 允许 `orderbook_price` 单独关闭 ROI 对片大小的放大

##### use_accel_mult
- **类型**: `bool`
- **默认值**: `true`
- **说明**: 动态数量策略是否启用公共 `accel_mult`
- **用途**: 允许 `orderbook_price` 单独关闭短窗动能对片大小的放大
...
- **推荐原因**:
  ...
  - `5m` 当前最适合作为 primary regime judgment
  - `15m` 更适合作为滞后确认，不适合做第一判断
- **影响**:
  - 更小：状态机更敏感，但更容易抖动
  - 更大：状态机更稳，但更滞后
- **建议**:
  - 先保持默认 `5m`
  - 后续根据 `market_data_*.jsonl` 离线回放和历史统计再调参

#### pressure_regime_samples
- **类型**: `int`
- **默认值**: `12`
- **说明**: `PRESSURE_REGIME` 至少积累多少个统计窗口样本后开始判定
- **当前默认语义**: 默认按最近 `12` 个 `5m` 样本判定，约等于最近 `60m`
- **当前推荐值**: `12`
- **推荐原因**:
  - 少于这个值时，状态机更容易被短时噪音误导
  - 明显高于这个值时，状态机会更稳，但会更晚感知 regime shift
  - `12 × 5m ≈ 60m` 是当前线上观察与离线分析之间的折中起点
- **影响**:
  - 更小：更早输出 regime，但噪音更大
  - 更大：更稳，但更晚感知 regime shift
- **建议**:
  - 先保持默认 `12`
  - 后续根据离线回放和线上样本复核再调整

#### pressure_regime_resume_enabled
- **类型**: `bool`
- **默认值**: `true`
- **说明**: 启动时是否尝试恢复最近一次 `PRESSURE_REGIME` 的滚动快照与状态缓存
- **当前默认语义**:
  - 若上次保存的状态与当前配置口径一致，且每个 `symbol|side` 最近一条 regime snapshot 距离当前时间未超过 `pressure_regime_resume_max_gap_ms`，则直接续用该 key 的最近样本
  - 若状态文件过旧、窗口不一致或样本数不一致，则自动跳过恢复，从头积累
- **建议**:
  - 保持开启，避免短暂停机/重启后再次等待接近 `12 × 5m`

#### pressure_regime_resume_max_gap_ms
- **类型**: `int`
- **默认值**: `900000`
- **单位**: 毫秒
- **说明**: 允许恢复 `PRESSURE_REGIME` 状态的最长样本间隔；按每个 `symbol|side` 最近一条 snapshot 单独判断
- **当前默认语义**: 默认 `900000ms = 15m`
- **当前推荐值**: `900000`（`15m`）
- **推荐原因**:
  - 足以覆盖短暂停机、重启、发版重拉起等常见场景
  - 又不会把过旧的 regime 快照硬接到新的行情阶段
- **影响**:
  - 更小：更保守，恢复机会更少，但更不容易误用旧状态
  - 更大：连续性更强，但跨 regime 误恢复的风险更高
- **建议**:
  - 先保持默认 `15m`
  - 后续结合离线回放和线上样本，再评估是否放宽到 `20-30m`
...
- **类型**: `boolean`
- **默认值**: `true`
- **说明**: 风险触发通知（强制平仓、保护性止损，以及 `PRESSURE_REGIME` 进入 `degrading / failed` 时的预警）
...
可按交易对覆盖 global 配置中的任意参数，**未指定的字段使用 global 默认值**。<br>
运行时 symbols 自动发现，`symbols` 仅作为覆盖，不作为订阅或执行白名单。
...
- **说明**:
  ...
  - `orderbook_pressure`：启用盘口量平仓路径；同一 symbol 上与 `orderbook_price` 互斥，不并行运行；其基准片大小默认也来自 `execution.base_mult`，并可按配置显式启用公共 ROI/accel sizing modifiers
...
symbols:
  "DASH/USDT:USDT":
    ...
    pressure_exit:
      ...
      qty_jitter_pct: 0.15       # 最终下单量围绕基准片大小双边抖动 15%
