<!-- Input: pytest 测试用例 -->
<!-- Output: 模块行为与回归验证（含暂停管理器与 Bot 命令控制） -->
<!-- Pos: tests 文件夹级说明与索引 -->
<!-- 一旦我所属的文件夹有所变化，请更新我。 -->
<!-- 一旦我被更新，务必更新我的开头注释，以及所属文件夹的MD。 -->
# tests 目录说明

pytest 测试集（含撤单分层、成交率反馈、保护止损、暂停控制回归）。<br>
覆盖配置、信号、执行、风控、WS 解析、通知格式与 Bot 命令。<br>
运行前无需外部依赖。
...
- `test_main_shutdown.py`：优雅退出 + 命令解析 + 保护止损调度竞态测试
...
- `test_pause_manager.py`：PauseManager 暂停状态管理测试（16 用例）
- `test_telegram_bot.py`：TelegramBot 命令接收器测试（14 用例）
