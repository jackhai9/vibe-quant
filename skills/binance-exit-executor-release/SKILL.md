...
4. Run `uv run pyright src/` when code changed.
5. Run targeted tests or full `uv run pytest -q` depending on blast radius.
6. Summarize what changed, what was verified, what was not verified, and residual risks.

## Validation Heuristics

- 小范围单模块改动：优先跑对应测试文件
- 跨 execution / exchange / risk / ws 边界的改动：至少补 `uv run pyright src/` 和多模块测试
- 涉及入口、配置、共享模型、状态机主路径：优先考虑全量 `uv run pytest -q`

## Release Checklist

- docs-as-code 已同步
- `uv run pyright src/` 已跑，或明确说明为什么没跑
